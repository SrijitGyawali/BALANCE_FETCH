"""
Polygon balance client using The Graph API.

This module provides Polygon-specific balance fetching functions using The Graph API.
...
async def get_native_matic_balance_thegraph(
    ...
    chain_info = get_chain_info("polygon")
    if not chain_info:
        return {
            "error": "Polygon network info not found",
            "token_type": "native",
            "token_symbol": "MATIC",
            "token_address": "0x0",
            "balance": "0",
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if data.get("data") and len(data["data"]) > 0:
                ...
                balance_str = f"{balance_matic:.18f}".rstrip("0").rstrip(".")
        ...
        return {
            "error": f"Error fetching MATIC balance: {str(e)}",
            "token_type": "native",
            "token_symbol": "MATIC",
            "token_address": "0x0",
            "balance": "0",
...
async def get_token_balance_polygon_thegraph(
    account_address: str, token_symbol: str, api_key: Optional[str] = None
) -> Dict:
    """
    Get balance for a specific ERC-20 token on Polygon using The Graph API.
    ...
    all_tokens = await get_multiple_token_balances_polygon_thegraph(
    ...
    for token in all_tokens.get("tokens", []):
        ...
        if isinstance(token_sym, str) and token_sym.upper() == token_symbol_upper:
            ...
            if "chain_name" not in token:
                token["chain_name"] = "Polygon"
            if "chain_id" not in token:
                token["chain_id"] = "137"
            return token
    
    return {
        "error": f"Token {token_symbol.upper()} not found for {account_address} on Polygon",
        "token_type": "ERC20",
        "token_symbol": token_symbol.upper(),
        "token_address": "",
        "balance": "0",
...
async def get_multiple_token_balances_polygon_thegraph(
    account_address: str, token_symbols: Optional[List[str]] = None, api_key: Optional[str] = None
) -> Dict:
    """
    Get all ERC-20 token balances on Polygon using The Graph API.
    
    Matches the signature pattern of existing balance clients:
    - get_multiple_token_balances_polygon(account_address: str, token_symbols: list[str]) -> list[dict]
    ...
    page = 1
    limit = 10
    max_pages = 50  # Reduced from 100 to limit total requests (matches Ethereum)
    ...
    while page <= max_pages:
        try:
            ...
            await asyncio.sleep(0.5)  # Increased rate limiting delay to 0.5s (matches Ethereum)
            ...
            query_params = {
                ...
                "limit": str(limit),
                "page": str(page),
                "include_null_balances": "true",  # Changed to true to get ALL tokens (including zero balances)
            }
